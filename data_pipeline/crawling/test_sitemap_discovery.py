...
class SitemapProductUrlDiscovererTests(unittest.TestCase):
    ...
    def test_yoastseo_html_wrapper_triggers_googlebot_retry(self):
        """When server returns HTML wrapper (YoastSEO), fetcher should
        retry with Googlebot UA automatically."""
        yoastseo_html = (
            '<!DOCTYPE html><html><head><title>XML Sitemap</title></head><body>'
            'This XML Sitemap is generated by YoastSEO. '
            'It is meant for consumption by search engines.'
            '</body></html>'
        )
        real_sitemap = """
        <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
        ...
        class RetryFakeFetcher:
            ...
            def __call__(self, url: str) -> str:
                ...
                if len(self.calls) == 1:
                    return yoastseo_html
                return real_sitemap

        fetcher = RetryFakeFetcher()
        discoverer = SitemapProductUrlDiscoverer(fetcher=fetcher, max_sitemaps=5)
        ...
        self.assertIn("https://shop.example/products/sofa-1/", urls)
        self.assertIn("https://shop.example/products/table-1/", urls)
        self.assertEqual(len(fetcher.calls), 2, "Should retry once with Googlebot")

    def test_yoastseo_wrapper_gives_up_after_second_failure(self):
        """If Googlebot retry also returns HTML wrapper, raise clear error."""
        html = (
            '<!DOCTYPE html><html><head><title>XML Sitemap</title></head><body>'
            'This XML Sitemap is generated by YoastSEO.</body></html>'
        )

        class AlwaysHtmlFetcher:
            def __call__(self, url: str) -> str:
                return html

        with self.assertRaises(SitemapDiscoveryError) as ctx:
            SitemapProductUrlDiscoverer(fetcher=AlwaysHtmlFetcher()).discover(
                "https://shop.example/sitemap.xml",
            )
        self.assertIn("Googlebot", str(ctx.exception))
